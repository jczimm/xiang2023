...
The challenges/opportunities for my learning in this replication project will include: 1) recreating the paper's proposed (joint effort) model and two alternative (solitary/compensatory effort) models of the probability of joint success in [Memo](https://github.com/kach/memo) (based on the authors' pre-existing implementations in WebPPL), and 2) running these models over my newly collected data to attempt to replicate that the proposed model is most predictive of participants' judgments.

Specifically, I will attempt to replicate that: 2.1) when rounds 1 and 2 feature no individual success, participants still judge the probability of joint success in round 3 as non-zero; 2.2) participants judge higher probability of joint success as cases of individual success increases; and 2.3) the proposed model is the only model evaluated which predicts both of these patterns (see Figure 3C in original paper and accompanying explanation, pp. 1570-1571). Note that 2.2 and 2.3 are exploratory, as they are not statistically formalized and tested in Xiang2023. I will also replace the original authors' data-saving step (a php page) with [DataPipe](https://pipe.jspsych.org/).
