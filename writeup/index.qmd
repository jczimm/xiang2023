...
The challenges/opportunities for my learning in this replication project will include 1) recreating the paper's proposed (joint effort) model and an alternative (solitary effort) model of the probability of joint success in [Memo](https://github.com/kach/memo) (based on the authors' pre-existing implementations in WebPPL), and 2) running these models over my newly collected data to attempt to replicate that the proposed model is more predictive of participants' reported joint success probability than the alternative model (see Figure 3C in original paper, p. 1571). I will also replace the original authors' data-saving step (a php page) with [DataPipe](https://pipe.jspsych.org/).
